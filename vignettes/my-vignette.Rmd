...
**Second step**. Other R packages are needed for running the  ConR package.
Normally these packages are installed automatically when installing the ConR package.
If not, this can be done by running the following code :
```
...
**Third step**. A proper way to work with R is to define a working directory. I advise to create a directory near the root (easier to handle), for example, here, named *Your_R_directory*
Now you can set your working directory by the following code:
```
setwd("C:/Your_R_directory/"")
```

**Fourth step**, copy the file *ConR_1.0.tar.gz* in this directory.
...
Note that the above procedures should be done only once, except the code for setting the working directory (third step).
...
You can change the file name for example by the name *MyResults* using the following code line:
```
...
It is possible to export in R the spatial polygon object used for estimating the EOO. This is done by using the argument `export_shp` which is a logical argument. A logical argument can take only two values: `T` for TRUE and `F` for FALSE.
...
Then, for the first species (mapped in grey):
...
For the second species (mapped in red)
...
```
  ...
  Note that after plotting the spatial polygon by the code ```plot(EOO.results[[2]][[2]]```, another spatial polygon is added as a **background** representing a default world land map. This map is loaded by the code `data(land)`.
...
In that case, occurrences within and outside protected area are threated separately by the same procedure: overlaying of grid of a given resolution.
```
data(Madagascar.protec)
